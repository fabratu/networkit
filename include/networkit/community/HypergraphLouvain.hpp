/*
 * HypergraphLouvain.hpp
 *
 * Created on: 07.2024
 *     Author: Isaline PLAID
 */

#ifndef NETWORKIT_COMMUNITY_HYPERGRAPH_LOUVAIN_HPP_
#define NETWORKIT_COMMUNITY_HYPERGRAPH_LOUVAIN_HPP_
...
double weight_1bis(double x) {
    return x / 2.0;
}

namespace NetworKit {

class HypergraphLouvain final : public CommunityDetectionAlgorithm<Hypergraph> {
public:
    /**
     *
     ...
     */
    HypergraphLouvain(const Hypergraph &graph, int iterations = 3, bool randomize = true,
                      double gamma = 1, int type_contribution = 1,
                      double (*weightFun)(double) = &weight_1bis);
