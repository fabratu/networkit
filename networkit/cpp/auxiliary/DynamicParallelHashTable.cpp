...
#pragma omp atomic read
        ...
        if (expected.key == ht_invalid_key) {
...
#pragma omp atomic read
        ...
        } else if (expected.key == ht_invalid_key) {
            return false;
        } else {
            idx++;
        }
...
cluster_range(HTAtomic128::Cells::const_iterator cells_begin,
              HTAtomic128::Cells::const_iterator cells_end, uint32_t const p_count,
              uint32_t const p_id) {
    ...
    if (p_id == (p_count - 1)) {
        ...
        while (p_cells_end != cells_end && p_cells_end->key != ht_invalid_key) {
            ++p_cells_end;
        }
    }
    ...
    if (p_id > 0) {
        ...
        while (p_cells_begin != p_cells_end && p_cells_begin->key != ht_invalid_key) {
            ++p_cells_begin;
        }
    }
...
#pragma omp single
    { target = std::make_unique<HTAtomic128>(source->capacity() << 1); }
    // omp implicit barrier

    roam(*source, *target, p_count, p_id);
#pragma omp barrier
...
HTHandle::~HTHandle() {
    ...
    unset_bit_atomically(m_sync_data.busy, m_sync_data.p_id);
...
HTCustodian::HTCustodian(size_t const begin_capacity) {
