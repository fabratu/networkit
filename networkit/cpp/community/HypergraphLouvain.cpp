...
namespace NetworKit {
HypergraphLouvain::HypergraphLouvain(const Hypergraph &graph, int iterations, bool randomize,
                                     double gamma, int type_contribution,
                                     double (*weightFun)(double))
    : CommunityDetectionAlgorithm(graph), gamma(gamma), numberOfIterations(iterations),
      random(randomize), type_contribution(type_contribution), weightFun(weightFun) {
...
double HypergraphLouvain::deltaModHypergraph(const Hypergraph &graph, const Partition &zeta,
                                             index S, const Partition &p, index c, index target_c) {
