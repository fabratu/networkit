...
double Modularity::getQualityHypergraph(const Partition &zeta, const Hypergraph &G, double gamma,
    ...
    if (type_contribution == 0) { // >>>>> STRICT EDGE CONTRIBUTION // FULLY WEIGHTED
        ...
        std::vector<double> EdgeSizeWeight(
            d + 1, 0.0); // vector of sum of weight of edges of size i (i=0 to d)
    ...
    if (type_contribution == 1) { // >>>>> MAJORITY EDGE CONTRIBUTION // FULLY WEIGHTED
        ...
        G.forEdges([&](edgeid eid, edgeweight ew) {
            isFirst = true;
            edgeBelongs = false;
            // an edge belongs to a community if the majority of its nodes belongs to the same
            // community
            std::vector<double> CommEdge(zeta.upperBound(), 0.0);
            for (auto nid : G.edgeMembers(eid)) {
                CommEdge[zeta[nid.first]]++;
            }
            for (auto v : CommEdge) {
        ...
        double sum_vol = 0.0;
        double sum = 0.0;
        // Compute expCov : \sum_d \frac{|E_d|}{vol(V)^d} \sum_{i= \frac{d}{2} +1}^{d}  \binom{d}{i}
        // \sum_{c \in C} (vol(c))^i (vol(V)- vol(c))^{d-i}
        for (double j = 0; j < d + 1; j++) {
            sum = 0.0;
    ...
    if (type_contribution == 10) { // >>>>> STRICT EDGE CONTRIBUTION // PARTIALLY WEIGHTED
        ...
        double totalEdgeWeight = 0.0;    // add all edge weights
        std::vector<double> EdgeSizeWeight(
            d + 1, 0.0); // vector of sum of number of edges of size i (i=0 to d)
...
double Modularity::deltaModularityHypergraph(const Partition &zeta, const Hypergraph &G,
                                             std::set<node> S, index target_c, double gamma,
                                             int type_contribution) {
    ...
    //  The volume of the old community c of nodes of S including the nodes of S   (All S nodes
    ...
    if (type_contribution == 0) { // >>>>> STRICT EDGE CONTRIBUTION
        ...
        for (edgeid eid : border_edge) {
            ...
            for (auto nid : G.edgeMembers(eid)) {
                if (!edgeBelongs_c
                    && (S.find(nid.first) == S.end())) { // eid is not strictly included in S, so
            ...
            for (auto nid : G.edgeMembers(eid)) {
                ...
                if (zeta[nid.first] != target_c && (S.find(nid.first) == S.end())) {
                    edgeBelongs_target_c = false;
                    break;
                }
            }
        ...
        // expCovGain = \sum_{d\geq 2} \frac{|E_d|_w}{|E|_w vol_w(V)^d}(vol_w(c'+S)^d -vol_w(c')^d
        // -vol_w(c)^d + vol_w(c-S)^d)
        for (double j = 0; j < d + 1; j++) {
            expCovGain += (EdgeSizeWeight[j] / (totalEdgeWeight * pow(volume, j)))
    ...
    if (type_contribution == 1) { // >>>>> MAJORITY EDGE CONTRIBUTION
        ...
        for (edgeid eid : border_edge) {
            ...
            for (auto nid : G.edgeMembers(eid)) {
                ...
                if (zeta[nid.first] == target_c) {
                    CommEdge[1]++;
                }
                if (S.find(nid.first) != S.end()) {
                    CommEdge[2]++;
                }
            }

            if (!(CommEdge[1] >= int(G.order(eid) / 2. + 1))
                && !(CommEdge[0] - CommEdge[2] >= int(G.order(eid)) / 2. + 1)) {
        ...
        for (double j = 0; j < d + 1; j++) {
            ...
            for (double i = int((j / 2.) + 1); i <= j; i++) {
                sum += BinomialCoefficient(j, i)
                       * (pow(vol_target_c + vol_S, i) * pow(volume - vol_target_c - vol_S, j - i)
                          - pow(vol_target_c, i) * pow(volume - vol_target_c, j - i)
                          - pow(vol_c, i) * pow(volume - vol_c, j - i)
                          + pow(vol_c - vol_S, i) * pow(volume - vol_c + vol_S, j - i));
            }
    ...
    if (type_contribution == 10) { // >>>>> STRICT EDGE CONTRIBUTION // PARTIALLY WEIGHTED
        ...
        for (double j = 0; j < d + 1; j++) {
            expCovGain +=
                (EdgeSizeWeight[j] / (totalEdgeWeight * pow(volume_unweight, j)))
                * (pow(vol_target_c_unweight + vol_S_unweight, j) - pow(vol_target_c_unweight, j)
                   - pow(vol_c_unweight, j) + pow(vol_c_unweight - vol_S_unweight, j));
        }
    }
    ...
    if (type_contribution == 11) { // >>>>> MAJORITY EDGE CONTRIBUTION // PARTIALLY WEIGHTED
        ...
        for (double j = 0; j < d + 1; j++) {
            ...
            for (double i = int((j / 2.) + 1); i <= j; i++) {
                sum += BinomialCoefficient(j, i)
                       * (pow(vol_target_c_unweight + vol_S_unweight, i)
                              * pow(volume_unweight - vol_target_c_unweight - vol_S_unweight, j - i)
                          - pow(vol_target_c_unweight, i)
                                * pow(volume_unweight - vol_target_c_unweight, j - i)
                          - pow(vol_c_unweight, i) * pow(volume_unweight - vol_c_unweight, j - i)
                          + pow(vol_c_unweight - vol_S_unweight, i)
                                * pow(volume_unweight - vol_c_unweight + vol_S_unweight, j - i));
            }
