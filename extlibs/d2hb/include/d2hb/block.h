...
#ifndef D2HB_SYSTEM_ALLOCATOR
class BlockHandle {
  public:
    BlockHandle() : m_ba{nullptr}, m_index{0} {}

    BlockHandle(BlockArray* ba, size_t index) : m_ba{ba}, m_index{index} {}
...
class BlockArray {
  public:
    BlockArray(size_t bytes_per_entry, size_t bsize, BlockCache* cache, unsigned int age)
        : m_age{age}, m_bytes_per_entry(bytes_per_entry), m_cache(cache), m_bsize(bsize) {
        size_t const min_capacity = 0x200000; // 2 MiB.
        size_t const min_blocks = 10;
    ...
    size_t bytes_per_entry() const { return m_bytes_per_entry; }
    ...
    void* access_storage_of(size_t index) {
        return static_cast<char*>(m_data) + index * block_bytes();
    }

    index_type* access_htab_of(size_t index) {
        if (uses_htab(m_bsize))
  ...
  private:
    ...
    void* m_data;
    index_type* m_htab;
    std::vector<size_t> m_alloc_stack;
    std::vector<size_t> m_free_stack;
    const size_t m_bytes_per_entry;
    const size_t m_bsize;
    size_t m_capacity;
    size_t m_nblocks;
};

inline size_t BlockHandle::bsize() { return m_ba->bsize(); }
...
inline index_type index_hash_table(uint64_t const hash, index_type const current_index,
                                   size_t const bsize) {
...
template <typename E> class BlockState {
  public:
    ...
    Vertex degree() const { return m_degree; }
    ...
    std::tuple<iterator, bool> insert(Vertex v, E ed) {
        ...
        if (uses_htab(m_bsize)) {
            index_type* const htab = m_htab;
            uint64_t const h = hash64(v);
            index_type j;
    ...
    const_iterator valid_cend() const { return const_iterator(m_entries + m_degree); }
...
inline void BlockArray::recycle() {
    for (size_t const index : m_free_stack) {
