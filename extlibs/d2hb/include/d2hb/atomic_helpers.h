...
template <typename T> void swap_atomics_non_atomically(std::atomic<T>& a, std::atomic<T>& b) {
    T const a_temp = a.load();
    a.store(b);
    b.store(a_temp);
};

template <typename T> T increment_atomically(std::atomic<T>& value, T increment = T(1)) {
